...
const he = require("he");
const staticTypes = require("./static");
const userDiscord = require("discord.js");
...
async function generateTranscript(messages, inputChannel, opts = {
    ...
    if (opts.useCDN) {
        ...
        link.setAttribute('href', 'https://cdn.jsdelivr.net/npm/discord-html-transcripts@' +
            (version ?? 'latest') +
            '/dist/template.css');
    ...
    await Promise.all(messagesArray.map(async (message) => {
        ...
        if (message.reference?.messageId) {
            ...
            <span class="chatlog__reference-name" title="${author.username.replace(/"/g, '')}" style="color: ${author.hexAccentColor ?? '#FFFFFF'}">${author.bot
        ...
        if (message.attachments && message.attachments.size > 0) {
            for (const attachment of Array.from(message.attachments.values())) {
                ...
                if (['png', 'jpg', 'jpeg', 'gif'].includes(attachmentType)) {
                    ...
                    attachmentImage.src =
                        (opts.saveImages
                            ? await (0, utils_1.downloadImageToDataURL)(attachment.proxyURL ?? attachment.url)
                            : null) ??
                            attachment.proxyURL ??
                            attachment.url;
    ...
    if (opts.returnType === 'attachment')
        return new Attachment(Buffer.from(serialized)).setName(opts.fileName ?? 'transcript.html');
...
function formatContent(content, context, allowExtra = false, replyStyle = false, purify = he.escape) {
    ...
    content = purify(content)
        ...
        const userInGuild = isDJSv14
            ? context.client?.users?.resolve(userId)
            : context.client?.users?.resolve(userId);
        ...
        const channelInGuild = isDJSv14
            ? context.guild.channels.resolve(channelId)
            : context.guild.channels.resolve(channelId);
        let isText = false;
        let isVoice = false;
        if (channelInGuild !== null) {
            isText = isDJSv14
                ? (channelInGuild)?.isTextBased()
                : channelInGuild?.isText();
            isVoice = isDJSv14
                ? (channelInGuild)?.isVoiceBased()
                : channelInGuild?.isVoice();
        }
        ...
        const roleInGuild = isDJSv14
            ? context.guild.roles.resolve(roleId)
            : context.guild.roles.resolve(roleId);
        if (!roleInGuild)
